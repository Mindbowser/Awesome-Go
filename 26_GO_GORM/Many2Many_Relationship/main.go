...
func main() {
	...
	//Fetching
	user := &UserL{}
	db.Where("uname=?", "Ray").Find(&user)
	err := db.Model(&user).Association("Languages").Find(&user.Languages).Error
	fmt.Println("User is now coming", user, err)

	//Deletion
	fmt.Println(user, "to delete")
	db.Where("uname=?", "John").Delete(&user)

	//Updation
	db.Model(&UserL{}).Where("uname=?", "Ray").Update("uname", "Martin")
